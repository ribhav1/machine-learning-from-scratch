...
Y = np.array([14, 23, 32, 41, 50, 59, 68])
...
# list to store how the mean squared error changes over training iterations for visualization
mse_over_reps = []
...
def train(X_train: np.array, Y_train: np.array, weights: np.array, bias: float, lr: float, epochs: int, mse_over_reps: list):
    for epoch in range(epochs):
        ...
        mse_over_reps.append(mse(X_train, Y_train, weights, bias))

    # after all epochs return parameters and mse history
    return weights, bias, mse_over_reps
...
# train model
weights, bias, mse_over_reps = train(X_train, Y_train, weights, bias, lr, epochs, mse_over_reps)
